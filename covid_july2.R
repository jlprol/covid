...
# and countries with subhourly resolution corrected to average i.o. summing up to hourly
...
get_dload<- function(years = c(2015:2020), months = (1:12)){
  for(y in years){
    for(m in months){
      ...
      # CET time 15' (i.e. divide/4)
      c2<- raw_data %>% 
        filter(AreaName %in% c("Austria", # AT
                               "Germany", # DE
                               "Hungary", # HU
                               "Netherlands"  # NL
        )) %>%  
        ...
        mutate(local_datetime = format(datetime, tz= "Europe/Berlin")) %>%
        mutate(date = as.Date(substr(local_datetime, 1, 10))) %>%
        group_by(country, date) %>% summarise(load = sum(load)/4000) %>%
        droplevels() %>% ungroup() %>% as_tibble()
      
      # UK time hourly
      c3<- raw_data %>%
        filter(AreaName %in% c("Portugal"  # PT
        )) %>% 
        select(year = Year, month = Month, day = Day, datetime = DateTime, country = MapCode, 
               load = TotalLoadValue) %>% 
        mutate(datetime = as.POSIXct(datetime, tz = "UTC")) %>% 
      ...
      # UK time 30' (i.e. divide/2)
      c4<- raw_data %>%
        filter(AreaName %in% c("National Grid BZ", # GB
                               "Ireland" # IE
        )) %>% 
        select(year = Year, month = Month, day = Day, datetime = DateTime, country = MapCode, 
               load = TotalLoadValue) %>% 
        ...
        mutate(local_datetime = format(datetime, tz= "Europe/London")) %>%
        mutate(date = as.Date(substr(local_datetime, 1, 10))) %>%
        group_by(country, date) %>% summarise(load = sum(load)/2000) %>%
        droplevels() %>% ungroup() %>% as_tibble()
      
      # Eastern European time hourly
      c5<- raw_data %>%
      ...
      load<- bind_rows(c1, c2, c3, c4, c5) %>% mutate(country = as.factor(country)) %>% arrange(country, date)
...
strindexplot<- ggplot(str_data, aes(index, dif)) +
  ...
  geom_shadowtext(data = medpoint, aes(x = index, y = dif, color = country, label = country), 
